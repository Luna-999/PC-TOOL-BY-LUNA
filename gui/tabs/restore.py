"""Restore tab — view change logs and restore active sessions."""
import threading
import subprocess
...
class RestoreTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        muted_label(self, "Review modifications and rollback system changes"
    ...
    def _manual_rp(self):
        self._status_lbl.configure(text="Creating manual restore point…", text_color=C.WARNING)
    ...
    def _refresh(self):
        ...
        try:
            ...
            for s in sessions:
                ...
                for c in s["changes"][:5]:  # show up to 5
                    row = ctk.CTkFrame(changes_frame, fg_color="transparent")
                    row.pack(fill="x", pady=2)
                    lbl = f"• [{c['change_type'].upper()}] "
                    if c.get("device_id"):
                        lbl += f"Device {c['device_id'][:30]}... "
                    lbl += f"→ {c['reg_value_name']}"
                    ctk.CTkLabel(row, text=lbl, font=(FONT_FAMILY, 12),
                                 text_color=C.TEXT).pack(side="left")

                if len(s["changes"]) > 5:
                    ctk.CTkLabel(changes_frame, text=f"... and {len(s['changes'])-5} more changes",
                                 font=(FONT_FAMILY, 12), text_color=C.MUTED).pack(anchor="w", pady=2)

                danger_button(card, "Revert Session", lambda sid=s["session_id"]: self._revert(sid),
    ...
    def _revert(self, session_id):
        self._status_lbl.configure(text="Reverting session…", text_color=C.WARNING)
        threading.Thread(target=self._revert_worker, args=(session_id,), daemon=True).start()
    ...
    def _revert_worker(self, session_id):
        try:
            ...
            self.after(0, lambda: self._status_lbl.configure(
                text=f"Reverted {success_count}/{len(changes)} changes ✓", text_color=C.SUCCESS))
